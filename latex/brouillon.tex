...
Frobenius $\phi_q : [X, Y, Z] \to [X^q, Y^q, Z^q]$ qui a pour polynôme
...
$\textup{min}(\textup{ord}_{\ell}(\alpha),\textup{ord}_{\ell}(\beta))$ de 
$\GF{q}$ est la plus petite extension contenant un point d'ordre $\ell$ et 
l'extension de degré $\textup{ord}_{\ell}(\phi_q)$ de $\GF{q}$ est la plus
petite extension contenant tout $E[\ell]$.
%TODO: Préciser comment ça se fait..
...
% (déterminer l'élément + calcule de l'isomorphisme);
%

...
\GF{q}[\mu_m]/\GF{q}$ ?). On a en particulier l'inclusion
$\langle{q}\rangle\subset G\subset\zmodninv{m}$ avec 
\begin{equation}
\#G/\langle{q}\rangle=g\etmath\#\langle{q}\rangle = f = [K[\mu_m]:K]/g.
\end{equation}
...
$\langle{q}\rangle$ agit transitivement sur les $\eta(\zeta_m)$.\\
\end{proof}
Le but de la section va être de démontrer le théorème suivant :

\begin{thm}
\label{th:gausspernorm}
Soit $\GF{q^n}/\GF{q}$ une extension de degré $n$ et un entier $m$ tel que :
\vspace{0.3cm}
\begin{enumerate}[(i)]
    \item $\zmodninv{m} = \langle{q}\times S$ pour $S$ un sous-groupe,
    \item $(\varphi(m), r) = 1$,
    \item $m$ est sans facteur carré.
\end{enumerate}
\vspace{0.3cm}
Si $\#\langle{q}\rangle = n$ alors les périodes $\eta(\zeta_m)$ forment une base

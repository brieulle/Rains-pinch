...
rapport deux variantes d'un algorithm dû à Rains, l'une dite cyclotomique et
l'autre dite elliptique, lui-même tirer d'un algorithme de Pinch, pour 
déterminer des isomorphismes entre corps finis de même cardinal.
...
Le cardinal de $(S_{\mathfrak{p}}S)/S_{\mathfrak{q}}$ est la même pour tous les
...
$\textup{min}(\ord{\ell}{\alpha},\ord{\ell}{\beta})$ de 
$\GF{q}$ est la plus petite extension contenant un point d'ordre $\ell$ et 
l'extension de degré $\ord{\ell}{\phi_q}$ de $\GF{q}$ est la plus
petite extension contenant tout $E[\ell]$.\par
...
Elkies ou un puissance d'un tel nombre premier semble être la meilleure
solution. Si on note $T_E$ la trace du Frobenius d'une courbe elliptique $E$,
alors $m$ premier ou puissance d'un premier de Elkies entraine :
\begin{equation}
...
Il faut aussi pouvoir sélectionner la bonne courbe elliptique. La méthode qu'on
utilise est assez simple pour être expliquée succintement. On commence par
...
\ref{sec:singcurve} sont parmi les bons candidats associés à l'entier $m$. Si
aucune des dix courbes testées ne correspond, alors on teste chaque courbe de
$j$-invariant différent de $0$ et $1728$ et leurs tordues quadratiques en
parcourant tout $\GF{q}$ jusqu'à tomber sur une qui marche (Ça serait bien
d'avoir une preuve que ça arrive forcément.. Peut-être \cite{CasHen} ?).\par
Une fois la courbe et $m$ obtenus, on peut exécuté l'algorithme de Rains
proprement dit, \emph{cf} algorithme \ref{alg:gaussell}.
%TODO: expliquez un peu les trucs où l'algorithme
\begin{algorithm}
...
condition du nombre de trace admissible pour un $m$. Comme pour la méthode
cyclotomique, au départ on cherche un $m$ premier de la forme $an + 1$. 
Le traitement est différent selon si $m$ est égal à $p$ ou non.\par
...
qu'un seul bon candidat pour la trace, on peut imaginer en demande $d>1$. L'idée
serait que cela puisse améliorer la probabilité avec laquelle on peut
directement tomber sur une bonne trace (pertinent ? Si la distribution est
uniforme, je ne sais pas si ça changerait grand chose...).\par
Comme on choisit les courbes sur le corps de base $\GF{q}$, si $q$ est petit
...
De plus, le degré de l'extension $n$ doit diviser
...
l'étude de ces critères (avec plus de temps.. ?) afin de déterminer quelle
méthode utiliser dans un algorithme général qui regrouperait les deux méthodes.

...
$w$ en fonction des $y^i$ ce qui prend $O(...)$ opérations. Cependant, en
pratique les éléments sont déjà exprimés en fonction de la base monomiale. 
...
l'image de $x$ \emph{via} diverses techniques.\par
...
Comme pour la méthode cyclotomique, on calcule alorsce qu'on appelle les 
\emph{périodes elliptiques} qui consistent à sommer les abscisses de l'orbites 
d'un point de $m$-torsion sous l'action du groupe de Galois, ce sera détaillé 
dans la section \ref{sec:perell}. On aura alors obtenu deux éléments reliés et
accessoirement normal, il suffira alors de procéder comme dans la méthode 
cyclotomique \ref{sec:algcompcycl} afin d'obtenir l'isomorphisme désiré.

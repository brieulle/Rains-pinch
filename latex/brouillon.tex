...
\end{rem}
\begin{center}
\includegraphics[width=8cm, height=5cm]{EllipticCurves}\\
Courbes elliptiques sur $\RR$ - Source : 
...
On suppose qu'on a déjà trouver une courbe elliptique dont la classe de la trace
du Frobenius modulo $m$ est égale à un des $t$ satisfaisant les conditions
ci-dessus, alors $E(\GF{q^n})$ a bien des points de $m$-torsion. En effet,
puisqu'on a $\alpha,\beta\in\zmodninv{m}$ alors
$\alpha^n,\beta^n\in\zmodninv{m}$. Donc 
\begin{equation}
(X - \alpha^n)(X - \beta^n) = X - (\alpha^n + \beta^n)X + \overline{q}^n\bmod{m}
\end{equation}
puisque $\alpha\beta = \overline{q}$, et donc comme $\alpha$ est d'ordre $n$ :
\begin{equation}
1 + (\alpha^n + \beta^n) + \overline{q^n} = (1 - \alpha^n)(1 - \beta^n) =
0\bmod{m}.
\end{equation}
Or, ceci est exactement la classe modulo $m$ du nombre de points de
$E(\GF{q^n})$, d'où le résultat.\par
...
\begin{itemize}
    ...
    $\zeta_6$ une racine \nroot{$6$} primitive de l'unité :
        ...
        \end{align}
La méthode qu'on utilise pour sélectionner la courbe elliptique sur laquelle on
...
\ref{sec:elldef} sont parmi les bons candidats associés à l'entier $m$

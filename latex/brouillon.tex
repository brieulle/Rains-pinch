...
idéal de $K$ et soit $\mathfrak{p}\EO_L = \prod_{i=1}^{g}{\mathfrak{B}_i^{e_i}}$
sa décomposition en idéaux premiers dans $L$ avec chaque $\mathfrak{B}_i$
distincts. On a appelle \emph{indice de ramification} de $\mathfrak{B}_i$ sur 
$\EO_K$ la valeur 
\begin{equation}
e(\mathfrak{B}_i\vert \mathfrak{p}) := e_i
\end{equation}
et le \emph{degré d'inertie} ou \emph{degré résiduel} de $\mathfrak{B}_i$ sur
$\EO_K$ la valeur
\begin{equation}
f(\mathfrak{B}_i\vert \mathfrak{p}) := f_i =
[\EO_K/\mathfrak{B}_i:\EO_K/\mathfrak{p}].
\end{equation}
\end{defn}
...
$\mathfrak{p}$ un idéal de $K$ et $\mathfrak{p}\EO_L = 
\prod_{i=1}^{g}{\mathfrak{B}_i^{e_i}}$ sa décomposition en idéaux premiers dans
$\EO_L$. Alors le groupe $\textup{Gal}(L/K)$ agit transitivement sur
$\mathfrak{B}_1,\dots,\mathfrak{B}_g$, en particulier $e_1 = \dots = e_g$ et
$f_1 = \dots = f_g$. Si de plus on pose $e = e_1$ et $f = f_1$, alors on a :
\begin{equation}
\mathfrak{p}\EO_L = \mathfrak{B}_1^e\dots\mathfrak{B}_g^e
\end{equation}
...
$\mathfrak{B}/\mathfrak{p}$, %($= \GF{q}[\mu_m]/\GF{q}$ ?)
...
Soit $\GF{q^n}/\GF{q}$ une extension de degré $n$ et un entier $m$ premier avec
$q$ tel que :
\vspace{0.3cm}
\begin{enumerate}[(i)]
...
sur $\EO_{m,p}$. Soit maintenant $S$ un sous-groupe de $\zmodninv{m}$, on va 
étudier la forme de l'invariant $\EO_{m,p}^S$  de $\EO_{m,p}$ sous l'action de 
$S$, réduction modulo $\mathfrak{p}$ de $\EO_m^S$ l'invariant de $\EO_m$ sous
l'action du groupe $S$.\par
On a $\mathfrak{p} = \prod_{i=0}^g{\mathfrak{B}_i}$, où $g$ est le nombre
d'idéal de $\EO_m$ au-dessus de $p$. En effet, l'extension est galoisienne et 
que $p$ est ramifié dans $\EO_m$ puisque $m$ et $p$ sont premiers entre eux, 
théorème \ref{th:entiercycl}. Ainsi on a :
\begin{equation}
\EO_{m,p}= \EO_m/p\EO_m = \EO_m/\prod_{i=0}^g{\mathfrak{B}_i}
\end{equation}
D'après le théorème des restes chinois on obtient finalement :
\begin{equation}
\EO_{m,p} = \bigoplus_{i=0}^g{\EO_m/\mathfrak{B}_i}
\end{equation}
Or, les $\EO_m/\mathfrak{B}_i$ sont des extensions de corps de $\ZZ/p\ZZ \simeq
\GF{p}$ de même degré, le degré résiduel $f = [\EO_m/\mathfrak{B}_i:\GF{p}]$. 
Dans ce cas, on a :
\begin{equation}
\label{eq:structinvSmodp}
\EO_{m,p}^S = \bigoplus_{i=0}^{g'}{\GF{p^f}}
\end{equation}
où $g'$ est le nombre d'idéaux de $\EO_m^S$ au-dessus de $p$.
...
on a $\varphi(m) = gf$ définis comme ci-dessus. En prenant en compte que le
corps résiduel $\EO_m/p\EO_m$ est un corps fini, l'action du groupe de Galois
est uniquement constitué d'éléments de la forme $\sigma_{p^k}(x) = x^{p^k}$.
Mais cela implique que son degré est donc l'ordre de $p$ dans $\zmodninv{m}$ ce
qui donne le résultat pour $S = \groupgen{1}$ car dans ce cas :
\begin{equation}
g = \varphi(m)/f = [\zmodninv{m}:\groupgen{p}].
\end{equation}

Considérons maintenant $S$ comme un sous-groupe quelconque. Pour n'importe quel
$\mathfrak{p}$ de $\EO_m$, l'idéal premier en-dessous de lui dans $\EO_m^S$ est
l'idéal :
\begin{equation}
\prod_{i\in(S_{\mathfrak{p}}S)/S_{\mathfrak{p}}}{\sigma_i(\mathfrak{p})\cap\ZZ}
\end{equation}
où $S_{\mathfrak{p}}$ est le stabilisateur de $\mathfrak{p}$. En prenant $i\in 
S_{\mathfrak{p}}S$ on obtient tous les conjugués de $\mathfrak{p}$ et en
quotientant par $S_{\mathfrak{p}}$ on supprime les multiplicités plus grandes 
que $1$; on ne prend qu'une partie de la norme : 
\begin{equation}
N_{\QQ[\mu_m]/\QQ}(\mathfrak{p}) =
\prod_{i\in\zmodninv{m}}{\sigma_i\left(\mathfrak{p}\right)\cap\ZZ} = (p^f)
\end{equation}
idéal de $\ZZ$. Réciproquement, si $i\notin S_{\mathfrak{p}}S$ alors 
$\sigma_i(\mathfrak{p})$ est au-dessus d'un autre idéal premier que 
$p$. Le cardinale de $(S_{\mathfrak{p}}S)/S$ est la même pour tous les
$\mathfrak{p}$ au-dessus de $p$ puisqu'ils sont conjugués. Alors le nombre de
premier de $\EO_m^S$ au-dessus de $p$ est égal à :
\begin{equation}
...
l'ordre de $p$ dans $\zmodninv{m}$.
\end{cor}
\begin{proof}
Il suffit d'appliquer le lemme \ref{lem:nbprimeabove} et l'équation
(\ref{eq:structinvSmodp}).\\
\end{proof}
En particulier si $[\zmodninv{m}/S:\groupgen{p}] = 1$ alors $\EO_{m,p}^S$ est 
corps, c'est en fait exactement $\GF{p^n}$ avec $\#\groupgen{p} = n$. Reste 
alors à montrer que les périodes de gauss engendrent effectivement 
$\EO_{m,p}^S$.
\begin{lem}
...
\GF{p^n}$ et si $m$ est sans facteurs alors ils l'engendrent d'après le lemme 
précédent, ce sont donc des élements normaux de $\GF{p^n}$. La condition (ii) 
implique qu'ils engendrent aussi $\GF{q^n}$ et sont donc des éléments normaux de
celui-ci. Ce qui achève la preuve du théorème \ref{th:gausspernorm}.

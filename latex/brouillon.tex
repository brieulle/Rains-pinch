...
L'algorithme \ref{alg:convert} calcule correctement l'image de $x$ à partir de
deux éléments normaux $v$ et $w$ reliés entre eux en 
\begin{equation}
O(n\E{n}\,\textup{log}\;q)
\end{equation}
opérations sur $\GF{q}$.
...
Comme pour la méthode cyclotomique, on calcule alors ce qu'on appelle les 
\emph{périodes elliptiques} qui consistent à sommer les abscisses de l'orbites 
d'un point d'ordre $m$ sous l'action du groupe de Galois, ce sera détaillé 
...
\begin{defn}
Soit $E/\GF{q}$ une courbe elliptique, un nombre premier $m$ est appelé
nombre \emph{premier d'Elkies} pour $E$ si le polynôme minimal de $\phi_q$ se 
scinde en deux facteurs distincts dans $\zmodn{m}$.
\end{defn}
\begin{rem}
...
ne peuvent pas être laissés de côté (pourquoi ?). 
...
\begin{conj}
Soient $m$ et $t$ deux entiers tels que :
\vspace{0.3cm}
\begin{enumerate}[1.]
    \item $|t| < \sqrt{2q}$,
...
\end{enumerate}
\vspace{0.3cm}
Soit $E/\GF{q}$ une courbe elliptique de trace $t$ et soit $k$ la plus petite
extension de $\GF{q}$ contenant un point d'ordre $m$. Alors pour tout $P$
d'ordre $m$, les périodes elliptiques $\eta_{\alpha}(P)$ forment une base normal
de $k$ sur $\GF{q}$.
\end{conj}

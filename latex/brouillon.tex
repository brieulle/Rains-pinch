...
\end{equation}
On a aussi $f\in o(g)$ si et seulement si pour tout $\epsilon>0$ il existe une constante $N$ telle que :
\begin{equation}
...
\end{equation}
pour tout $n, m\in\NN$. En particulier, elle est \emph{superlinéaire}, \emph{cf.}
\cite{GaGe} :
\begin{equation}
...
\begin{defn}
    ...
    entre eux} si $\theta$ est un isomorphisme :
    \begin{equation}
...
éléments sont reliés entre eux s'ils ont le même polynôme minimal sur $k$, la 
seule différence est de niveau formelle.\par
De façon abstraite, $k_1$ et $k_2$ sont une même entité, le corps à $q^n$ 
éléments. Lorsqu'on descend d'un niveau d'abstraction et qu'on construit
concrètement un corps à $q$ éléments, on obtient des représentations 
différentes. Dans ce cas, $k_1$ et $k_2$ sont différents et par conséquent 
l'isomorphisme qui les relie n'est pas exactement un automorphisme. On peut donc
faire une différence entre être reliés et être conjugés.
...
petit de $k_1$ et $k_2$ respectivement, tels que $m$ petit tel que
$\#\groupgen{q} = n.o$. On peut alors trouver deux racines primitives
$m$-ième de l'unité $\zeta_{m}$ et $\zeta'_m$ qui engendrent $k'_1$ et $k'_2$
respectivement et tels que $\theta(\zeta_m) = \zeta'_m$. Alors on a : 
\begin{equation}
...
l'isomorthetasme en exigant que les générateurs satisfassent une condition plus 
...
et $w\in k_2$ tels que $\theta(v) = w$. Pour finir de résoudre le problème, il 
faut déterminer les images $\theta(x^i)$ pour $0\leq i < n$. Or comme 
$\theta(x^i) = \theta(x)^i$, il suffit même de déterminer uniquement l'image de 
$\theta(x)$.\par
...
alors on aura :
\begin{align*}
\theta(x) &= \sum_{i\in I}{c_i\theta(v)^{q^i}}\\
&= \sum_{i\in I}{c_iw^{q^i}}
\end{align*}
puisque $\theta$ est un morthetasme. Devoir passer de la base normale à la base 
...
$\#E(\GF{q^n}) = q^n + 1 - (\alpha^n + \beta^n)$, voir par exemple 
\cite[th. 2.3.1, chap V]{Sil}.\par
...
nombre \emph{premier d'Elkies} pour $E$ si le polynôme minimal de $\phi_q$ se 
scinde en deux facteurs distincts dans $\zmodn{\ell}$.
\end{defn}
\begin{rem}
Les premiers $\ell$ tels que le polynôme minimal de $\phi_q$ ne se scinde pas
dans $\zmodninv{\ell}$ sont appelés des nombre premier de Atkin.

\end{rem}
Si $\ell$ est un premier de Elkies, alors il existe une base de $E[\ell]$ sur 

...
Prenons $j\neq0, 1728$, si on considère $E$ sur un corps $K$ non algébriquement
clos, on prends $c\in K\setminus K^2$ et $c\neq0$ et on pose $u = \sqrt{c}$ 
et on définit la courbe $E^u$ telle que si $E : y^2 = x^3 + ax + b$ alors :
\begin{equation}
E^u : y^2 = x^3 + u^4ax + u^6b.
\end{equation}
On appelle $E^u$ la \emph{tordue quadratique} de $E$. En particulier, elles ont 
le même $j$-invariant puisqu'elles sont isomorphes sur $\overline{K}$, elles le
sont même sur une extension quadratique de $K$. Mais elles ne le sont pas sur 
$K$ directement, ce sont donc bien deux courbes différentes sur $K$.\par
Dans les cas $j = 0$ et $j = 1728$, il y a respectivement $6$ et $4$
automorphismes sur $\overline{K}$, on peut donc s'attendre à ce qu'il y ait $6$
et $4$ tordues différentes d'une courbe $E/K$.\par
Considérons alors $j = 1728$, alors la courbe de $j$-invariant correspondante 
est :
\begin{equation}
E : y^2 = x^3 + x.
\end{equation}
On prend $c$ un non-résidu quartique dans $K$ et on pose $u = \sqrt[4]{c}$,
alors on définit une \emph{tordue quartique} $E^u$ :
\begin{equation}
E : y^2 = x^3 + u^4x.
\end{equation}
On l'appelle quartique puisque pour que $E$ soit à coefficient dans $K$, il
suffit que $u$ soit une racine quatrième de l'unité, comme nous sommes un corps,
il y a donc quatres $u$ possibles pour définir chaque tordues.\par
Pour le cas $j = 0$, la courbe associée est :
\begin{equation} 
E : y^2 = x^3 + 1.
\end{equation}
Si on prend cette fois-ci $c$ un non-résidu sextique et qu'on pose $u =
\sqrt[6]{c}$, on obtient alors une \emph{tordue sextique} $E^u$ :
\begin{equation}
E^u : y^2 = x^3 + u^6
\end{equation}
qui pour les mêmes raisons évoques ci-dessus va de pair avec six autres tordues
de $E$, ce qui justifie à nouveau l'appelation.
...
même relié la trace du Frobenius de ces tordues.
\begin{prop}
\label{prop:trtwist}
...
dans le schéma \ref{} dû à Luca De Feo (communication personnelle). Elles jouent
donc un rôle analogue aux racines primitives $m$-ièmes de l'unité pour la 
méthode cyclotomique.\par
...
on suppose qu'on a déjà la courbe elliptique $E/\GF{q}$ dont on a besoin.\par
...
revient à calculer une racine carré dans $\GF{q^n}$ ce qui est en $O(...)$ 
opérations sur $\GF{q}$. Il est plus judicieux de prenre un $x$ au hasard dans 
...
O(n\,\E{n}\,\textup{log }q)
\end{equation}
opérations d'après \ref{}. Par la suite, il faut alors faire une multiplication
...
$3$GHz et de $1.23$go de \bsc{RAM}. Les algorithmes ont été implantés et 
...
tableau \ref{tab:resnumcyclavecext} se trouve une partie des valeurs numériques 
de l'exécution dont le temps d'exécution était le plus élevé.
...
\begin{tabular}{|c|c|c|}
	...
	\hline
\end{tabular}
\caption{Temps d'exécution de la méthode cyclotomique lorsque $o > 1$}
...
\begin{tabular}{|c|c|c|}
	...
	\hline
\end{tabular}
\caption{Temps d'exécution de la méthode cyclotomique lorsque que $o = 1$}

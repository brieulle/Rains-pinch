...
%TODO: tu dois encore le prouver
...
%TODO: Foutre le schéma qui passe de la carac 0 à la carac finie.
...
Avant toute chose, il faut pouvoir trouver un entier $m$, sans facteurs
carrés, petit, premier avec $q$ et tel que $o=o_m(q)$, l'ordre multiplicatif de 
$q$ modulo $m$, soit un multiple de $n$. Une fois un tel $m$ trouvé, on veut 
qu'il satisfasse aussi :
\vspace{0.3cm}
\begin{enumerate}[(1)]
\item $(n, o/n) = 1$,

\item $(n, \varphi(m)/(n.o)) = 1$.
\end{enumerate}
\vspace{0.3cm}
La condition (1) est nécessaire car on va travailler sur des extensions de degré
$o/n$ de $\GF{q^n}$, elle permet alors d'utiliser des polynôme à coefficients 
dans le corps premier. La condition (2) est là pour nous assurer de pouvoir 
appliquer les résultats du point précédent, \textit{i.e.} pour pouvoir trouver
un $\mathcal{K}$ tel que $\langle{q,\mathcal{K}}\rangle = 
\zmodninv{m}$ et donc que $\#\langle{q}\rangle = n.o$.\par
On peut résumer le point \ref{sec:pergauss}, avec les bonnes hypothèses, par le 
diagramme suivant :
...
{ &K[\zeta_m] & \GF{q}[\zeta_m] & \\
\QQ[\zeta_m]& & &\GF{p}[\zeta_m]\\
& K & \GF{q}& \\
\QQ& & &\GF{p}\\};
\path[-,font=\scriptsize,>=angle 90]
...
\end{tikzpicture}
\end{center}
où $K$ est un corps de nombres, $\mathfrak{p}$ un idéal premier de
$\mathcal{O}_K$ au-dessus de $p$ et $\mathfrak{B}$ un idéal premier de
$\mathcal{O}_{K[\zeta_m]}$ au-dessus de $\mathfrak{p}$. Les bonnes hypothèses
étant que $\mathfrak{p}$ soit tel que $\mathcal{O}_K/\mathfrak{p} = \GF{q}$ et
que $\mathfrak{p}$ ne soit pas ramifié dans $K[\zeta_m]/K$ ce qui équivaut à 
$p$ ne divise pas $m$ d'après \textit{(le résultat que je mettrais probablement 
un jour dans le rapport)}, ce qui est le cas puisque $(m, q) = 1$.
...
sq^a\bmod m$ pour un $s\in\mathcal{K}$ et $0 \leq a < 0$, alors $q = 

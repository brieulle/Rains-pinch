...
On en déduit que la trace est surjective
...
anneaux d'entiers et des idéaux sur ses anneaux. Le but va être de démontrer
l'identité fondamentale dans le cadre des extensions Galoisienne. Cela sera 
utile pour la sous-section \ref{sec:pergauss} et plus précisément le théorème
\ref{th:gaussnormal}.
...
, on aura besoin de ne manipuler que des matrices de taille $o$ 
...
utiliser les périodes de Gauss, introduites à la sous-section \ref{sec:pergauss}.
...
donc une unité du second; et comme la réduction de $1$ est toujours $1$ modulo 
quoique ce soit, une unité de $\GF{p}[X]/(X^n - 1)$ est nécessairement une unité
de $\GF{p}[X]/(X^{p^kn} - 1)$.\par
...
\label{sec:pergauss}
Maintenant qu'on connait les éléments qui nous intéresse, il faut trouver
un moyen de les rendre "uniques". Globalement, cela revient à
...
cyclotomiques (puisqu'il se décompose dans $\GF{q^n}$). Ce sont les périodes
elliptiques qui vont alors nous permettre de générer ces éléments. Commençons
par introduire une définition générale :
...
\end{thm}
On verra sur la fin que ce n'est possible que si $m$ est sans facteurs carrés,
...
de Gauss $\alpha$ et $\alpha'$ sont conjugués sur $\GF{q}$. Ce qui implique 
qu'on pourra très bien prendre deux racines primitive différentes, \textit{i.e.}
dans le sens où si on les appelle $\zeta$ et $\zeta'$, on ait pas directement
$\phi(\zeta) = \zeta'$; cela évite, entre autre, la recherche de
la bonne puissance de $\zeta'$ et tout ce qui en découle :\\
\textit{(Note : pas très sûr si c'est clair ou si ça implique ce que j'essaie 
d'exprimer)}
...
\sum_{a\in\mathcal{K}}{q(\zeta^{aq^j})} =
\left(\sum_{a\in\mathcal{K}}{g(\zeta^a)}\right) = \alpha^{q^j}\]

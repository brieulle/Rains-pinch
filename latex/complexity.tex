...
\end{tikzpicture}
\end{center}
La flêche $\pi^{\prime}$ est plus ou moins gratuite, il suffit d'exprimer $w$ en fonction des $y^i$ mais en général les éléments sont déjà exprimés en fonction de la base polynomiale. La seule flêche qui pose un problème est celle correspondant à $\pi$ qui permet d'exprimer $x$ en fonction des $v^{p^i}$. C'est elle que l'algorithme va calculer; ou plus exactement, l'algorithme va déterminer directement les coefficients $c_i$. Le morphisme $\phi$ se détermine alors simplement en calculant l'image de $x$.
\begin{defn}
...
Pour commencer, on peut rappeler un résultat classique d'algèbre linéaire qui énonce que toute forme linéaire peut s'exprimer sous la forme :%TODO : Classique mais tu sais pas le démontrer, alors faudra faire un ptit effort coco !
\[x \mapsto \Tr{\alpha x}\]
...
Or la trace est surjective,%TODO : c'est pas très clair dans ta tête alors faudrait que tu l'expliques un peu

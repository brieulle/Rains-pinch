...
\ref{sec:extension}. Il faut alors construire des élements normaux reliés par un
isomorphisme entre $k_1$ et $k_2$ ou leurs extensions, ce qui est fait 
\textit{via} les périodes de Gauss dans la section \ref{sec:gaussper}.\par
...
\subsection{Utilisation d'une extension}% Utile ?
\label{sec:extension}
...
l'unité $\zeta_m$, on définit la période de Gauss suivante :
\begin{equation}
...
%\ref{th:gaussnormal} 
pour calculer une période de Gauss $\eta(\zeta_m) = 
